<?php
	$goTo = new GetModel();
	$adminID = $goTo -> get_account_id($_SESSION['technicianUsername'], $_SESSION['technicianPassword']);
...
<?php
	if (isset($_POST['save'])){
		...
		# Verify if typed current password is equals to original password:
			if($_POST['currentPassword'] == $_SESSION['technicianPassword']) {
					...
					if($_POST['newPassword'] == $_POST['currentPassword']) {
						...
						if($isValid) {
							...
							if($isUpdated) {
								...
								echo "<script>technicianPasswordChanged()</script>";
							}
						} else {
