...
$adminID = $goTo -> getAdminIDByUsernameAndPassword($_SESSION['technicianUsername'], $_SESSION['technicianPassword']);
...
<table id='accountpasswordusername'>
	<form method='POST'>
		<tr>
			<td>Username &nbsp;</td>
			<td><input class='input' disabled='true' type='text' value='<?php echo $_SESSION['technicianUsername']?>'/></td>
		</tr>
...
<?php
	if (isset($_POST['save'])){
		...
		if($_SESSION['technicianUsername'] == $_POST['newUsername']) {
			...
			if($isValid) {
				...
				if($isUpdated) {
					...
					echo "<script>technicianAccountUpdated()</script>";
				}
			} else {
				echo "<script>alert('Username is already in use!');</script>";
			}
