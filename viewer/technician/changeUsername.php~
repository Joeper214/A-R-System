...
$adminID = $goTo -> getAdminIDByUsernameAndPassword($_SESSION['cashierUsername'], $_SESSION['cashierPassword']);
...
<table id='accountpasswordusername'>
	<form method='POST'>
		<tr>
			<td>Username &nbsp;</td>
			<td><input class='input' disabled='true' type='text' value='<?php echo $_SESSION['cashierUsername']?>'/></td>
		</tr>
...
<?php
	if (isset($_POST['save'])){
		...
		if($_SESSION['cashierUsername'] == $_POST['newUsername']) {
			...
			if($isValid) {
				...
				if($isUpdated) {
					...
					echo "<script>CashierAccountUpdated()</script>";
				}
			} else {
				echo "<script>alert('Username is already in use!');</script>";
			}
